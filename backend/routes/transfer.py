...
def delete_progress(type, transferID, server_type):
    if type == "download":
        if config.getting_files[f"{transferID}"]:
            config.getting_files.pop(f"{transferID}")
        if server_type == "ftp" and config.getting_files_byte[f"{transferID}"]:
            config.getting_files_byte.pop(f"{transferID}")
    elif type == "upload":
        if config.putting_files[f"{transferID}"]:
            config.putting_files.pop(f"{transferID}")
        if server_type == "ftp" and config.putting_files_byte[f"{transferID}"]:
...
def downloadHandler(src, dest, downloadID, server_type, sftp):
    ...
    dir_check = stat.S_ISDIR(
        sftp.stat(src).st_mode) if server_type == "sftp" else config.ftp_host.path.isdir(src)
    # if a directory
    if dir_check:
        ...
        if not os.path.exists(newfolder):
            os.mkdir(newfolder)
        for x in (sftp.listdir(src) if server_type == "sftp" else config.ftp_host.listdir(src)):
    ...
    # if not a directory
        ...
        if server_type == "sftp":
            ...
            callback_downloadID = functools.partial(
                byte_transferred_ftp, "download", downloadID, filename, config.ftp_host.path.getsize(src))
            config.ftp_host.download(src, os.path.join(
                dest, filename), callback_downloadID)
...
def uploadHandler(src, dest, uploadID, server_type, sftp):
    ...
    if os.path.isdir(src):
        ...
        newfolder = os.path.join(dest, filename)
        sftp.mkdir(newfolder) if server_type == "sftp" else config.ftp_host.mkdir(
            newfolder)
    ...
    # if not a directory
        ...
        if server_type == "sftp":
            ...
            config.ftp_host.upload(src, os.path.join(
                dest, filename), callback_uploadID)
...
def sftpput():
    ...
    if server_type == "sftp":
        ...
        destination_folder = os.path.dirname(content['destFile']) if not config.ftp_host.path.isdir(
            content['destFile']) else content['destFile']
    try:
